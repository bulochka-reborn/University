...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    float a = std::abs(mathX) * -1;
                    float b = 0.1 * mathX * mathX - 4;

                    if (mathY == 0.1 * mathX * mathX - 4 || mathY == std::abs(mathX) * -1) {
                        zone = -1;
                    } else {
                        if (mathY < std::abs(mathX) * -1 && mathY < 0.1 * mathX * mathX - 4) {
                            zone = 2;
                        } if (mathY < std::abs(mathX) * -1 && mathY > 0.1 * mathX * mathX - 4) {
                            zone = 1;
                        }
                        else if (mathY > std::abs(mathX) * -1 && mathY > 0.1 * mathX * mathX - 4) {
                            zone = 3;
                        }
                        else if (mathY > std::abs(mathX) * -1 && mathY < 0.1 * mathX * mathX - 4 && mathX > 0) {
                            zone = 5;
                        }
                        else if (mathY > std::abs(mathX) * -1 && mathY < 0.1 * mathX * mathX - 4 && mathX < 0) {
                            zone = 4;
                    ...
                    if (zone == -1){
                        coordinatesText.setString("Coordinates: (" + std::to_string(mathX) + ", " + std::to_string(mathY) + "); Zone: granica");
                    }
                    else {
                        coordinatesText.setString("Coordinates: (" + std::to_string(mathX) + ", " + std::to_string(mathY) + "); Zone: " + std::to_string(zone));
                    }
                }
            }
        }
        ...
        drawGraph(window, [](float x) { return std::abs(x) * -1; }, -10, 10, 30, 30, sf::Color::Blue);

        drawGraph(window, [](float x) { return 0.1 * x * x - 4; }, -10, 10, 30, 30, sf::Color::Red);
